...
const ipxSetup = (setupOptions) => (providerOptions, moduleOptions) => {
  ...
  const nuxt = useNuxt();
  const ipxBase = moduleOptions.base || "/_ipx";
  const hasUserProvidedIPX = nuxt.options.serverHandlers.find((handler) => handler.route?.startsWith(ipxBase)) || nuxt.options.devServerHandlers.find((handler) => handler.route?.startsWith(ipxBase));
...
${providers.map((p) => `  ['${p.name}']: { provider: ${p.importName}, defaults: ${JSON.stringify(p.runtimeOptions)} }`).join(",\n")}
    ...
    nuxt.hook("nitro:init", async (nitro) => {
      if (!options.provider || options.provider === "ipx" || options.provider === "ipxStatic") {
        const resolvedProvider = nitro.options.static || options.provider === "ipxStatic" ? "ipxStatic" : nitro.options.node ? "ipx" : "none";
